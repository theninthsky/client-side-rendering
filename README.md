...
_Note that if you are using CSS-in-JS in your app, you should [disable the speedy optimization](src/utils/disable-speedy.ts) for Chromium during prerendering in order to omit your styles to the DOM._
...
There are various examples of how this delayed functionality negatively impacts the user experience, like the way some websites only show the navigation bar after JS has been loaded (since they cannot access the localStorage to check if it has a user info entry).
